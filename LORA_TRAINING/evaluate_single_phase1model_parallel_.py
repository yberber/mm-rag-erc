...
def get_data_dirs(args):
    ...
    if args.dataset in ['iemocap', 'both']:
        data_dirs.append("TRAINING_DATA/PHASE1/IEMOCAP/")
    if args.dataset in ['meld', 'both']:
        data_dirs.append("TRAINING_DATA/PHASE1/MELD/")
    return data_dirs
...
def get_eval_data(args):
    ...
    for data_dir in data_dirs:
        ...
        data_files = {
            f"{args.split}": os.path.join(data_dir, f"{args.split}.jsonl"),
        }
...
def build_output_path(args):
    ...
    if args.adapter_path:
        ...
        output_dir = os.path.join("EVAL_PHASE1", adapter_subpath, split)
    else:
        output_dir = os.path.join("EVAL_PHASE1", "BASE", split)

    # Ensure output directory exists
    if not os.path.exists(output_dir):
        utils.makedirs(relative_path_from_project=output_dir)
