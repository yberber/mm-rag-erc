...
from utils import anonymize_speakers_in_dialog, mapped_emotion_to_id
from utils import set_pandas_display_options

set_pandas_display_options()
# --- Configuration Constants ---
VECTOR_STORE_BASE_PATH = Path("../vectorstore_data")
COLLECTION_NAME_PREFIX = "meld_iemocap_flow"
EMBEDDING_MODEL = "all-MiniLM-L6-v2"
MELD_DATA_PATH = Path("../../BENCMARK_DATASETS/meld_erc_with_categories.csv")
IEMOCAP_DATA_PATH = Path("../../BENCMARK_DATASETS/iemocap_erc_with_categories.csv")


def parse_arguments() -> argparse.Namespace:
    """Parses command-line arguments."""
    parser = argparse.ArgumentParser(
        description="Create a vector store with conversational flow as page content."
    )
    parser.add_argument(
        "--num_utterances",
        type=int,
        default=7,
        help="Number of utterances for the conversational flow.",
    )
...
def load_and_prepare_iemocap(file_path: Path) -> pd.DataFrame:
    ...
    df['speaker'] = df['speaker'].map({'F': 'Speaker_F', 'M': 'Speaker_0'}).fillna(df['speaker'])
    return df


def format_window_content(dialog_df: pd.DataFrame, turn_id: int, window_size: int) -> Tuple[str, List[int]]:
    """
    Formats a window of conversation into a single string and an emotion flow list.
...
def process_dialogue_group(
    ...
    for turn_id, row in dialog_group.iterrows():
        ...
        page_content, emotion_flow = format_window_content(dialog_group, turn_id, window_size)
        metadata = {
...
def main():
    """Main function to orchestrate the creation of the flow vector store."""
    args = parse_arguments()
    window_size = args.num_utterances

    print(f"--- Starting Vector Store Creation for Conversational Flow (Window Size: {window_size}) ---")
    ...
    try:
        ...
        print(f"\n✅ Successfully created the flow-based vector store!")
