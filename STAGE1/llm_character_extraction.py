...
@timing
def generate_characteristics_via_chain(chain, dataset, dataset_name, verbose=True):
    """
    ...
    for split in dataset.keys():
        for data in tqdm(dataset[split], desc=f"Creating speaker characteristics for {dataset_name} on split {split}"):

            speaker_characteristics = chain.invoke({**data["inputs"]})
            data["output"] = speaker_characteristics

            if verbose:
                print(f"Input prompt:\n{list(chain)[1][1].format(**data['inputs'])}")
                print(f"=> Output: {speaker_characteristics}  (idx: {data['iden']}, emotion: {data['emo']})")
                print("*****************************")
...
def main(config_dict=None):
    ...
    try:
        ...
        # Run tests
        dataset, stats = generate_characteristics_via_chain(
            chain,
            dataset,
