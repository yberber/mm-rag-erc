...
def get_base_parser():
    ...
    parser.add_argument('--model_name', type=str, default=constants.BASE_LLM_MODEL, help='Base model name')
    parser.add_argument(
...
class BaseTrainer:
    ...
    def _resolve_path_value(self, path_value):
        if path_value is None:
            return None
        path_obj = Path(path_value)
        if not path_obj.is_absolute():
            path_obj = paths.PROJECT_PATH / path_obj
        return path_obj
    ...
    def _process_config(self, config_dict):
        ...
        # Resolve data paths
        args.iemocap_data_path = str(self._resolve_path_value(args.iemocap_data_path))
        args.meld_data_path = str(self._resolve_path_value(args.meld_data_path))
        if args.stage1_adapter_path:
            args.stage1_adapter_path = str(self._resolve_path_value(args.stage1_adapter_path))
        ...
        if args.output_dir is None:
            ...
            output_dir = paths.ARTIFACTS_DIR / "finetuning" / stage_name / dataset_name / lora_type
        else:
            output_dir = self._resolve_path_value(args.output_dir)
    ...
    def save_config(self, save_directory, checkpoint):
        ...
        utils.dump_json_test_result(config_dict, path=config_path, add_datetime_to_filename=True)
    ...
    def _load_data_from_dirs(self, data_dirs):
        ...
        for data_dir in data_dirs:
            ...
            data_files = {
                "train": dir_path / "train.jsonl",
                "dev": dir_path / "dev.jsonl",
            }
    ...
    def run(self):
        ...
        try:
            ...
            output_dir_path = Path(self.config.output_dir)
            final_checkpoint_dir = output_dir_path / "final_checkpoint"
            ...
            if output_dir_path.is_dir():
                checkpoints = [p for p in output_dir_path.glob("checkpoint-*") if p.is_dir()]
                if checkpoints:
            ...
            self.save_config(output_dir_path, checkpoint)
            ...
            self.trainer.save_model(str(final_checkpoint_dir))
