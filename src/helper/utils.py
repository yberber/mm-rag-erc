"""Shared utility functions and data constants for the project.

Contents
--------
- Emotion label sets and ID mappings for MELD, IEMOCAP, and their union.
- Emotion-mapper dictionaries that convert raw dataset labels to a canonical
  label set (e.g. ``"joy"`` → ``"joyful"``).
- I/O helpers: JSON load/save, dataset-as-DataFrame loaders, path resolution.
- Model loaders for Ollama, HuggingFace pipeline, and VertexAI (Gemini).
- Miscellaneous utilities: timing decorator, speaker anonymisation,
  ChromaDB collection checks, string-to-bool conversion.
"""
...
def set_pandas_display_options():
    """Configure pandas to display all rows, columns, and full precision."""
...
def get_mapped_emotion_set(dataset=None):
    """Return the valid mapped emotion labels for a given dataset.

    Args:
        dataset (str, optional): One of ``"meld"``, ``"iemocap"``,
            ``"both"``, or ``None``.  ``None`` and ``"both"`` return the
            full union set.

    Returns:
        list[str]: Ordered list of mapped emotion label strings.

    Raises:
        ValueError: If ``dataset`` is not a recognised value.
    """
...
def extract_emotion_from_llm_output(output_text, valid_emotions=emotion_set,
    ...
    """Extract a valid emotion label from raw LLM output text.

    Args:
        output_text (str): Raw text generated by the LLM.
        valid_emotions (list[str]): Accepted emotion labels.  Defaults to
            the union emotion set.
        approach (EmotionExtractionStrategy): Extraction strategy.

            - ``FirstMentionedValidLabel``: return the first valid emotion
              that appears (leftmost position) in the output.
            - ``OneMentionedValidLabel``: return the label only if exactly
              one valid emotion is mentioned; otherwise return a sentinel.

    Returns:
        str: The extracted emotion label, or ``"NoValidEmotionFound"`` /
            ``"MultipleValidEmotionsFound"`` when extraction fails.
    """
...
def get_path(path=None, relative_path_from_project=None):
    """Resolve exactly one of an absolute path or a project-relative path.

    Args:
        path (str, optional): Absolute file-system path.
        relative_path_from_project (str, optional): Path relative to the
            project root.

    Returns:
        Path: Resolved absolute :class:`pathlib.Path`.
...
def load_json_multiline(path):
    ...
    Returns:
        list: List of Python objects parsed from each line.
    """
...
def dump_json_test_result(result, path=None, relative_path_from_project=None, add_datetime_to_filename=False, verbose=True):
    ...
    Args:
        result (object): JSON-serialisable Python object to save.
        path (str, optional): Absolute output path.
        relative_path_from_project (str, optional): Output path relative to
            the project root.
        add_datetime_to_filename (bool): If ``True``, insert a timestamp
            before the ``.json`` extension.  Defaults to ``False``.
        verbose (bool): Print the output path after writing.  Defaults to
            ``True``.
    """
...
def anonymize_speakers_in_dialog(df_dialog, use_letters=False):
    """Replace speaker names with anonymous identifiers in a dialogue DataFrame.

    Both the ``speaker`` column and any occurrences of speaker names inside
    the ``utterance`` column are replaced (case-insensitively, whole-word
    matching).

    Args:
        df_dialog (pd.DataFrame): DataFrame for a single dialogue containing
            at least ``"speaker"`` and ``"utterance"`` columns.
        use_letters (bool): If ``True``, use letter-based IDs
            (``Speaker_A``, ``Speaker_B``, …) instead of numeric ones
            (``Speaker_0``, ``Speaker_1``, …).  Defaults to ``False``.

    Returns:
        pd.DataFrame: The modified DataFrame with anonymised speaker names.
    """
...
def get_dataset_as_dataframe(dataset_name, splits=None, columns=None):
    """Load the final benchmark CSV for MELD or IEMOCAP as a DataFrame.

    Args:
        dataset_name (str): ``"meld"`` or ``"iemocap"`` (case-insensitive).
        splits (str | list[str], optional): Split(s) to keep.  ``None``
            returns all splits.
        columns (list[str], optional): Subset of columns to return.
            ``None`` returns all columns.

    Returns:
        pd.DataFrame: Filtered DataFrame from the final benchmark CSV.
...
def get_meld_iemocap_datasets_as_dataframe(splits=None, return_only_dataset=None):
    """Load and optionally filter both MELD and IEMOCAP final benchmark CSVs.

    Args:
        splits (str | list[str], optional): Split(s) to keep.  ``None``
            returns all splits.
        return_only_dataset (str, optional): If ``"meld"`` or ``"iemocap"``,
            return only that dataset's DataFrame.  ``None`` returns both.

    Returns:
        pd.DataFrame | Tuple[pd.DataFrame, pd.DataFrame]: A single DataFrame
            if ``return_only_dataset`` is set, otherwise a ``(meld_df,
            iemocap_df)`` tuple.
    """
...
def get_vectordb_path_from_attributes(db_type: str, max_m: int = None):
    """Construct the vector-store directory path from its type and window size.

    Args:
        db_type (str): One of ``"single"``, ``"flow"``, or ``"hybrid"``.
        max_m (int, optional): Window size (number of utterances); required
            for ``"flow"`` and ``"hybrid"`` types.

    Returns:
        Path: Path to the vector-store directory under
            ``artifacts/vectorstores/db/``.
...
def load_model_via_hf(model_id="meta-llama/Meta-Llama-3.1-8B-Instruct", max_output_tokens=10):
    """Load a text-generation model via the HuggingFace pipeline.

    Requires a CUDA-capable GPU.

    Args:
        model_id (str): HuggingFace model identifier.
        max_output_tokens (int): Maximum number of new tokens to generate.

    Returns:
        HuggingFacePipeline: LangChain-wrapped HF pipeline instance.
...
def load_model_via_ollama(model_id="llama3.1:8b", max_output_tokens=10):
    """Load a text-generation model served by a local Ollama instance.

    Args:
        model_id (str): Ollama model tag (e.g. ``"llama3.1:8b"``).
        max_output_tokens (int): Maximum number of tokens to predict.

    Returns:
        OllamaLLM: LangChain-wrapped Ollama LLM instance.
    """
...
def abstacted_audio_text(row):
    """Build the short audio-feature description string for a DataFrame row.

    Combines the categorical audio levels into a human-readable phrase used
    as the ``audio_features`` slot in emotion-recognition prompts.

    Args:
        row: A pandas Series (or dict-like) with ``"rate_level"``,
            ``"pitch_level"``, and ``"intensity_level"`` fields.

    Returns:
        str: A phrase such as
            ``"low speech rate, medium pitch, and high intensity"``.
    """
