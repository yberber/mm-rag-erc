...
from src.helper.utils import (load_model_via_ollama, load_model_via_hf,
                              get_dataset_as_dataframe, abstacted_audio_text, timing,
                              dump_json_test_result)
from src.config import constants, paths

CONCURRENCY_LIMIT = constants.CHARACTERISTICS_EXTRACTION_CONCURRENCY
...
def rel_path_to_save_results(args, dataset):
    ...
    return paths.SPEAKER_CHARACTERISTICS_DIR / data_name
...
def save_results(test_info, dataset, path_to_save):
    ...
    dump_json_test_result(test_result, path=path_to_save, add_datetime_to_filename=False)
...
async def main(config_dict=None):
    ...
    try:
        ...
        if path_to_save.exists():
            print(f"Results have already been saved at: {path_to_save}")
            return
